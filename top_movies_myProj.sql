...
How many movies from each studio? **/
SELECT Studio, COUNT(*) as amt_produced FROM topmovies
    GROUP BY Studio
...
SELECT COUNT(*) as amt_released, CASE
    ...
    END "released_in"
FROM topmovies
GROUP BY released_in;
/**Wait... what are those random 2 outliers??*/
SELECT Title, CASE
    WHEN Year >= 1980 AND Year < 1990 THEN Year
    WHEN Year >= 1970 AND Year < 1980 THEN Year
    END "released_in"
FROM topmovies
GROUP BY released_in;
/** ^Need to figure out why the final table entry keeps showing up... **/
